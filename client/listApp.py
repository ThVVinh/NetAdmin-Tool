...
class ListAppProcess():
    ...
    def update_tree(self, application_info):
        self.listView1.delete(*self.listView1.get_children())
        
        self.application_info = sorted(application_info, key=lambda info: info.split(', ')[0][6:])  # Sắp xếp theo tên

        for info in self.application_info:
    ...
    def kill_application(self):
        ...
        if selected_item:
            ...
            if item_data:
                pid = item_data[1]  # Lấy ID application (PID)
    ...
    def send_kill_command(self, pid):
        ...
        self.nw.flush()
        self.application_info = [info for info in self.application_info if not info.endswith(f"PID: {pid}")]
        self.update_tree(self.application_info)
    ...
    def view_application(self):
        ...
        for _ in range(num_application):
            ...
            application_info.append(info)
            
        self.update_tree(application_info)
